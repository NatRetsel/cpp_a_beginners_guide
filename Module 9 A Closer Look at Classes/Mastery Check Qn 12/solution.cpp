/*
    For the Set class developed in Project 9-1, define | so that it yields the symmetric difference between two sets.
    The symmetric difference consists of those elements that the two sets do not have in common.
*/
#include <iostream>
...
public:
    ...
    Set operator | (Set ob2); // symmetric difference
};
...
Set Set::operator | (Set ob2) {
    ...
    for (int i=0; i < len; i++) {
        if (ob2.find(members[i])==-1) temp = temp + members[i];
    } 
...
int main() {
    ...
    std::cout << "The symmetric difference of s1 and s2: " << std::endl;
    s4 = s1 | s2;
    s4.showset();

    std::cout << "The symmetric difference of s2 and s3: "<<std::endl;
    s4 = s2 | s3;
    s4.showset();
